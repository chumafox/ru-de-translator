...
class TTSEngine:
    ...
    def _speak_qwen(self, text: str, repeat: int = 1) -> None:
        ...
        def _play_chunks(chunks: list[bytes]) -> None:
            ...
            player = threading.Thread(
                target=self._player_thread, args=(audio_q, done_event), daemon=True
            )
            player.start()
            try:
                for chunk in chunks:
    ...
    async def _speak_say(self, text: str, repeat: int = 1) -> None:
        """Fallback to macOS built-in TTS."""
        for _ in range(repeat):
            if self._stop_event.is_set():
                break
            proc = subprocess.Popen(["say", "-v", "Anna", text])
            while proc.poll() is None:
                await asyncio.sleep(0.1)
                if self._stop_event.is_set():
                    proc.terminate()
